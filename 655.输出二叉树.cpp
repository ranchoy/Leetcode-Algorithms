...
public:
    vector<vector<string>> printTree(TreeNode *root) {
        ...
        build_tree_arr(root, ans, 0, (n-1)/2);

        return ans;
    }

    void build_tree_arr(TreeNode *root, vector<vector<string>> &ans, int i, int j) {
        ...
        if(root->left) build_tree_arr(root->left, ans, i+1, j-k);
        if(root->right) build_tree_arr(root->right, ans, i+1, j+k);
    }
